...
//        double tempdouble=static_timing(templevel,num1,g);
        ...
        int a=1;
        map <double,vector<int>> slackmap;
        for(int i=0;i<num;i++)
        {
            slackmap[g[i].slack].push_back(i);
        }
        map<double,vector<int>>::iterator slackmapit;
        ofstream slackfile;
        slackfile.open("slack.dat");
        slackfile<<"#This file is called slack.dat\n";
        slackfile<<"#Plots slack vs numgate per slack\n";
        slackfile<<"Slack\tNumgates/slack\n";
        
        for(slackmapit=slackmap.begin();slackmapit!=slackmap.end();slackmapit++)
            slackfile<<slackmapit->first<<"\t"<<slackmapit->second.size()<<"\n";
        slackfile<<"\n";
        slackfile.close();
